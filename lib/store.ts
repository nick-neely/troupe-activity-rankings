...
import type { ActivityData } from "./utils"
  ...
  persist(
    (set, get) => ({
      ...
      getTopActivities: (limit = 10) => {
        ...
        return [...activities].sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, limit)
      },
      ...
      getTotalStats: () => {
        ...
        const totalLoveVotes = activities.reduce((sum, activity) => sum + activity.love_votes, 0)
        const totalLikeVotes = activities.reduce((sum, activity) => sum + activity.like_votes, 0)
        const totalPassVotes = activities.reduce((sum, activity) => sum + activity.pass_votes, 0)
        const avgScore =
          totalActivities > 0
            ? activities.reduce((sum, activity) => sum + (activity.score || 0), 0) / totalActivities
            : 0
      ...
      getBudgetAnalysis: () => {
        ...
        const priceRanges = {
          Free: activities.filter((a) => a.price === "Free" || a.price === "$0" || a.price === "N/A"),
          ...
          .map(([range, rangeActivities]) => {
            const avgScore =
              rangeActivities.length > 0
                ? rangeActivities.reduce((sum, a) => sum + (a.score || 0), 0) / rangeActivities.length
                : 0
            const bestValue =
              rangeActivities.length > 0
                ? rangeActivities.reduce((best, current) => ((current.score || 0) > (best.score || 0) ? current : best))
                : null
      ...
      getGroupDynamics: () => {
        ...
        const consensus = activities.filter((a) => a.love_votes >= 2 && a.pass_votes === 0)

        const controversial = activities.filter(
          (a) => a.love_votes >= 1 && a.pass_votes >= 1 && Math.abs(a.love_votes - a.pass_votes) <= 1,
        )

        const polarizing = activities.filter((a) => a.pass_votes >= 2 && a.love_votes >= 1)

        const unanimous = activities.filter((a) => a.love_votes >= 3 && a.like_votes === 0 && a.pass_votes === 0)
      ...
      getCategoryLeaders: () => {
        ...
        return Object.entries(categories)
          .map(([category, categoryActivities]) => {
            ...
            const avgScore = categoryActivities.reduce((sum, a) => sum + (a.score || 0), 0) / categoryActivities.length
